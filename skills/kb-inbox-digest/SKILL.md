...
description: Convert shared links (from Telegram/Feishu/Discord/Obsidian inbox) into a tri-layer KB workflow: fetch content (web_fetch + browser fallback for WeChat), normalize fields, QA score, route to Obsidian Vault notes/queue, update processed state, and deliver a brief digest summary. Use when building or running the KB/Skills/Cron pipeline, debugging why a link did not get ingested, or when the user says “run the KB digest / process my links / 我又塞了链接 / 跑一下闭环”.
---
