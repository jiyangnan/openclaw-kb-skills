---
name: kb-fetch
description: Unified content fetching for KB ingestion. Fetches readable text from URLs with web_fetch first, and uses local browser (profile=openclaw) as a fallback for dynamic/login-walled sources like WeChat (mp.weixin.qq.com) and sometimes X. Use when a link cannot be fetched, when building KB ingest pipelines, or when the user says “抓取这篇/微信抓不到/用浏览器打开提取正文/提取#js_content”.
---
